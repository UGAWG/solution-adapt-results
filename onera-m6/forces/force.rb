...
coloring= %w(BLUE RED CUSTOM28 CUSTOM3 CUSTOM32 PURPLE)
...
File.open(mcr,'w') do |f|
  ...
  5.times do |i|
    ...
    f.printf linemap+"SYMBOLS{SYMBOLSHAPE{ASCIISHAPE{ASCIICHAR='#{code}'}}}\n"
    f.printf linemap+"ASSIGN{SHOWINLEGEND = AUTO}\n"
    f.printf '$!ACTIVELINEMAPS += ['+m.to_s+"]\n"
  end

  f.printf '$!LINEPLOTLAYERS SHOWSYMBOLS = YES'+"\n"
  ...
  f.printf axisdetail+"{RANGEMAX = 2.5}"+"\n"
  ...
  f.printf axisdetail+"{RANGEMIN = 70}"+"\n"
  f.printf axisdetail+"{RANGEMAX = 86}"+"\n"
  f.printf axisdetail+"{GRSPACING = 2.0}"+"\n"
